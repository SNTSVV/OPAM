...
#    -b <string>: base path for this tool, this path will be used to store task sets, no default
...
SIM_TIME=2000
CMD_PREFIX=""
TARGET_PATH="res/synthetics2"

# Generate synthetic task sets (EXP2.1: varying the number of tasks)
for ((nTask=10; nTask<=50; nTask+=5)); do
    ${CMD_PREFIX} java -jar artifacts/Synthesizer.jar -b ${TARGET_PATH}/1_nTasks -n ${N_TASKSETS} -s ${SIM_TIME} -c ${nTask} -m ${nTask}
done
...
for ((ratio=5; ratio<=50; ratio+=5)); do
    ...
    ${CMD_PREFIX} java -jar artifacts/Synthesizer.jar -b ${TARGET_PATH}/2_ratioAperiodic -n ${N_TASKSETS} -s ${SIM_TIME} -c ${ratioValue} -r ${ratioValue}
done
...
for ((rangeFactor=2; rangeFactor<=10; rangeFactor+=1)); do
    ...
    ${CMD_PREFIX} java -jar artifacts/Synthesizer.jar -b ${TARGET_PATH}/3_maxArrivalRange -n ${N_TASKSETS} -s ${SimulationTime} -c ${rangeFactor} -a ${rangeFactor}
done

# Generate synthetic task sets (EXP2.4: varying the simulation time)
for ((T=2000; T<=10000; T+=1000)); do
    ${CMD_PREFIX} java -jar artifacts/Synthesizer.jar -b ${TARGET_PATH}/4_simTime -n ${N_TASKSETS} -s ${T} -c ${T}
done