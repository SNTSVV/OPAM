...
public class PriorityNSGAII extends NSGAII<PrioritySolution> {
	...
	public PriorityNSGAII(int _cycle,
	                      ...
	                      SolutionListEvaluator<PrioritySolution> evaluator)
	{
		...
		if (this.cycle!=0) {
			externalFitness = new ExternalFitness(problem, (PrioritySolutionEvaluator) evaluator, _best, getMaxPopulationSize(), dominanceComparator);
		}

		init_fitness();
		init_maximum();
		
		if (PriorityNSGAII.searchedList==null) {
			PriorityNSGAII.searchedList = new UniqueList();
		}
		...
		for(int i = 0; i < matingPool.size(); i += numberOfParents) {
			...
			while(it.hasNext()) {
				...
				do {
					...
					if (PriorityNSGAII.searchedList.size() >= maxFact){
						flagFULL = true;
						break;
					}
				} while(!PriorityNSGAII.searchedList.add(s.toArray()));
				if (flagFULL==true) break;
...
//		JMetalLogger.logger.info("["+getName()+"] Iteration "+iterations+" (evaluated solutions: "+evaluations+")");
	...
	private void init_fitness(){
		// Title
		String title = "Cycle,Iteration,SolutionIndex,SolutionID,Schedulability,Satisfaction,DeadlineMiss,Rank,CrowdDistance"; //
