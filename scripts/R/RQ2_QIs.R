...
# RESULT_PATH <-sprintf('%s/results/RQ1-NSGA-Adaptive', PROJECT_PATH)
...
QIs       <- c("HV", "GDP", "CSunique", "GS")
QIs_NAMES <- c("HV", "GD+", "C", "Spread")

SUBJECTS <- c('ICS', 'CCS', 'UAV', 'GAP',  'HPSS', 'ESAIL')
SUBJOUTS <- c('ICS', 'CCS', 'UAV', 'GAP',  'HPSS', 'ESAIL')
APPRS<-c("SEQ", "OPAM")

SELECTED_TEST_NUM<- NULL      # Set for another external fitness (default: null, all points will be considered)
...
########################################################################
{
    ...
    dpath <- sprintf("%s/RQ2_%s", RESULT_PATH, optionname)
    ...
    for(measureID in c(1:length(QIs))){
        ...
        filename <- sprintf("%s/rq2-%s.pdf",dpath, QIs_NAMES[measureID])
