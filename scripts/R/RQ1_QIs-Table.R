...
# Table type 1
{
    ...
    cat("\\begin{table}[!p]\n")
    cat("\\color{rev}\n")
    cat("\t\\caption{\\color{rev}Comparing OPAM and RS using the four quality: HV, GD+, C, and $\\Delta$. Average quality values computed based on 50 runs of OPAM and RS using the different sets of task-arrival sequences (see Section~\\ref{subsec:design}).}\n")
    cat("\t%\\footnotesize\n")
    cat("\t\\vspace{-1.2em}\n")
    ...
    cat("\t\t\\colorbox{blue!30}{\\textbf{n.nnnn}}: OPAM outperforms RS \\quad\\quad \\colorbox{gray!20}{\\textbf{n.nnnn}}: RS outperforms OPAM}} \\\\\n")
    cat("\\end{tabularx}\n")
    cat("\\end{center}\n")
    cat("\\label{tbl:rq1QIs}\n")
    cat("\\end{table}\n")
    sink()
}
