...
${CMD_PREFIX} java -jar artifacts/OPAM.jar --getTest Adaptive --numTest ${NUM_TEST} -b ${TESTPOOL} --data ${RESOURCE} --cpus ${NCPU} --max ${SIM_TIME} --quanta ${TIME_QUANTA}


# Execute OPAM (${NUM_RUNS} runs will be executed)
${CMD_PREFIX} java -jar artifacts/OPAM.jar --runCnt ${NUM_RUNS} -b ${WORKPATH} --data ${RESOURCE} --testPath ${TESTPOOL}/test.list --numTest ${NUM_TEST} --cpus ${NCPU} --max ${SIM_TIME} --quanta ${TIME_QUANTA} --maxMissed 1000 ${OPTION}
